...
import com.kofikodr.kofipod.update.UpdateInfo
...
class UpdateRepositoryTest {
    ...
    fun markApkDownloaded_movesAvailableToReadyToInstall() =
        runHarnessTest {
            ...
            repo.markApkDownloaded("/data/data/com.kofikodr.kofipod/files/updates/kofipod-1.2.0.apk")

            val state = repo.state().first()
            assertTrue(state is UpdateUiState.ReadyToInstall, "expected ReadyToInstall, got $state")
            assertEquals(SAMPLE_INFO.version, state.info.version)
            assertEquals("/data/data/com.kofikodr.kofipod/files/updates/kofipod-1.2.0.apk", state.apkPath)
        }
