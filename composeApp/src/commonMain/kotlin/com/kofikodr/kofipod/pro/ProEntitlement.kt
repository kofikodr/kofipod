...
enum class ProSource {
    ...
    /**
     * Granted via the hidden Settings → tap-version-7× → enter-code affordance.
     * Used to give Play Store / sideload reviewers full access to Pro features
     * without a real purchase. Sticky and device-local (excluded from Auto Backup
     * via `kofipod_entitlement` prefs file). Distinguished from [Individual] so
     * analytics + UI can tell them apart.
     */
    ReviewerUnlock,
}
