 ...
 *
 * Returning a [Result] keeps error mapping uniform with the rest of the AI
 * surface — failures become an [AiError] and surface in
 * [DiscussRepository]'s transient-error channel.
 */
    ...
    override suspend fun loadContext(
        ...
        val url =
            episode.transcriptUrl?.takeIf { it.isNotBlank() }
                ?: return Result.success(DiscussContext.NotAvailable)
        return transcripts.fetch(url).map { body ->
            DiscussContext.Available(transcript = body, fingerprint = url)
        }
    }
}
