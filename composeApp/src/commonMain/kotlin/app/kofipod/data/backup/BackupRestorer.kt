...
class BackupRestorer(
    ...
    suspend fun restore(blob: BackupBlob): RestoreSummary = withContext(Dispatchers.Default) {
        var listsInserted = 0
        ...
        db.transaction {
            val existingListIds = db.podcastListQueries.selectAll().executeAsList()
                .associateBy { it.id }
            for (l in blob.lists) {
            ...
            val existingPodcastIds = db.podcastQueries.selectAll().executeAsList()
                .map { it.id }.toSet()
            ...
            for (s in blob.playbackStates) {
                val existing = db.playbackStateQueries.selectByEpisode(s.episodeId)
                    .executeAsOneOrNull()
                ...
                val ep = db.episodeQueries.selectById(s.episodeId).executeAsOneOrNull()
                    ?: continue
                @Suppress("UNUSED_VARIABLE") val _referenced = ep
