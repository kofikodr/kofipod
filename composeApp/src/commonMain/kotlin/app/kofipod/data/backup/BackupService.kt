...
class BackupService(
     ...
     */
    suspend fun runBackup(): BackupOutcome =
     ...
     */
    suspend fun readRemoteBlob(): RemoteBlob? =
        withContext(Dispatchers.Default) {
    ...
    private fun buildSnapshot(): BackupBlob {
        ...
        val lists =
            db.podcastListQueries.selectAll().executeAsList().map {
                BackupList(
                    id = it.id,
                    name = it.name,
        ...
        val podcasts =
            db.podcastQueries.selectAll().executeAsList().map {
                BackupPodcast(
                    id = it.id,
                    title = it.title,
                    author = it.author,
        ...
        val playback =
            db.playbackStateQueries.selectAll().executeAsList().map {
                BackupPlayback(
        ...
        val meta =
            SYNCED_META_KEYS.mapNotNull { key ->
    ...
    companion object {
         ...
         */
        val SYNCED_META_KEYS: Set<String> =
            setOf(
